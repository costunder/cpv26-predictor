...
## 날짜별 그래프: 버전 4 (기존 버전 2·3 읽기 지원)

v3의 박스스코어 입력은 2001~2022에만 채워지고 최근 타석 입력과 연결되지 않았습니다.
v4는 현재 cutoff에서 유효한 PA만 먼저 선택한 뒤 선수·경기·역할별 합계로 묶어 같은
...
두 출처가 겹치면 확인된 동일 선수는 박스스코어의 관측 필드를 우선하고 빈 필드만
PA로 채웁니다. 신원 미확정 행은 개인별로 병합하지 않습니다. 해당 경기·팀의 팀 통계와
LiveHit·집계 정답은 박스스코어를 우선하며 실제 PA 개인 이력과 PA 정답은 유지합니다.
원본을 삭제하거나 같은 경기·팀의 팀 통계/LiveHit를 두 번 합산하지 않습니다.
...
README 실행의 캐시는 `var/datasets/kbo_graph_2001_2026_v4/manifest.json`과
...
`box_batting_rows`, `box_pitching_rows` 등 박스스코어 사용 건수도 manifest에 기록합니다.
...
`var/datasets/kbo_graph_2001_2026_v4`를 만들고 새 run에서 시작합니다.

v4에서는 최근 관측 타석에서도 집계 정답을 만들므로 2025/2026의 `box_pa`와
