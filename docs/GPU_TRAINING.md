...
| 선수명·포지션·누적 승패/ERA/타율·ETC | 원문·감사 정보 보존, 확인되지 않은 사전 입력으로 사용하지 않음 |
...
사용하고 과거 팀별 타격·투구 집계를 prior로 줍니다. 이 자료로 선수 개인의 경력 연결을
완성했다고 해석하면 안 됩니다. 2023년 이후 실제 ID와 타석 경로는 그대로 유지합니다.
...
박스스코어 관측 노드에는 과거 팀-역할 prior를 주며, 당일 타격·투구 수치는 정답에만 둡니다.
팀별 2개 역할 prior 노드가 과거 집계를 전달하므로 과거의 모든 관측 ID를 계속 늘리지 않습니다.
더블헤더 첫 경기 결과도 같은 날 다른 경기의 역사 입력으로 들어가지 않습니다.

README 실행의 캐시는 `var/datasets/kbo_graph_2001_2026_full/manifest.json`과
...
`var/datasets/kbo_graph_2001_2026_full`을 만들고 새 run에서 시작합니다.
