...
처음 설치가 끝난 후 접속 방식과 무관하게 새 터미널·Bash 실행 환경을 열거나 학습을
재개할 때마다 다음 순서를 사용합니다. 아래 경로는 README의 clone 위치를 따른 예시이며,
다른 위치에 받았다면 실제 프로젝트 폴더로 이동합니다.
...
사용 데이터는 **KBO 공식 배포 데이터가 아니라**, NAVER Sports 중계를 가공해
...
2026년 부분 시즌은 이 기본 학습·검증·테스트 분할에 넣지 않습니다.
...
2023년에 실제로 수집해 둔 자료만으로 한 실시간 운영 재현은 아닙니다.
...
캐시는 기본 `var/datasets/kbo_graph/manifest.json`과 `days/YYYY-MM-DD.npz`에 저장합니다.
...
아래 수치는 기본 고정 스냅샷과 그래프 버전 2 기준입니다.
...
Linux CUDA 학습 경로는 구현돼 있지만 **실제 Linux GPU 실행은 아직 검증하지 않았습니다**.
Windows CPU 테스트·smoke의 세부 결과와 검증 범위는 한곳에서 관리합니다.
