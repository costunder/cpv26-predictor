...
- 프로젝트 버전: `0.4.0`
- DuckDB schema version: `5` (38 tables)
- 새 그래프 생성 버전: `5`; 기존 `2/3/4` 읽기 유지
- 실행 대상: Linux Bash와 활성화된 전용 Conda 환경
- 현재 검증 host: Windows CPU / Python 3.12.13

현재 실행 명령은 README의 v5 경로를 따른다. 아래 과거 실행 기록의 그래프·학습
경로를 새 실행에 섞지 않는다. 기존 v3/v4 환경에서는 원천 재다운로드나 환경 재설치 없이
캐시된 2023~2026 타석 파일을 `kbo-import`로 다시 적재한 뒤 새 그래프를 생성한다.
adapter v2는 더블헤더 번호를 새 행으로 보완하고 이전 원천 행은 보존한다.
새 원천 스냅샷을 고를 때 빠진 경기·타석이 옛 행에서 살아나지 않게 한다.
v5의 항목별 겹침 처리·사용량·수치 계산 수정 결과는
[오류 수정 검증](ERROR_FIX_VALIDATION.md)에 별도로 기록한다.
...
- `README.md`
  - 초보자가 Linux Bash에서 clone, CUDA 설치·검사, 데이터 적재·graph 생성,
...
python -c "import os, sys; from pathlib import Path; assert Path(sys.prefix).resolve() == Path(os.environ['CONDA_PREFIX']).resolve(); print(sys.executable)"
```
...
현재 그래프는 v4이다. v3의 역사 박스스코어/최근 PA 입력 단절을 고쳐 선수·경기별
관측 통계를 공통 입력에 연결한다. 미확정 역사 선수는 이름·팀·역할 그룹 prior를 쓰며
관측 ID를 개인 ID로 병합하지 않는다. 기존 v2/v3 그래프·체크포인트는 읽을 수 있고,
새 v4 데이터로 기존 학습을 재개하지 않는다. 입력 수정과 예측 성능 향상 검증은 구분한다.
