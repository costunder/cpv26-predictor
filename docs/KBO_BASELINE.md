...
이 문서는 기존 2023~2025년 CatBoost 비교 실험 기록이다. 현재 README의 RelGNN 학습은
2001년부터 시작하며, 추가된 역사 경기 원천은 [GPU 학습 설명](GPU_TRAINING.md#데이터-출처와-고정-스냅샷)에 있다.
...
이 문서에 기록한 기존 학습·평가는 완결된 2023~2025의 2,160경기·169,481 PA만 사용했다.
...
[README의 Conda 설치 절차](../README.md)에 따라 전용 환경 `cpv26`과 `.env`를 준비한다.
다음 명령으로 이 환경에 CatBoost를 추가한다. 기존 CUDA PyTorch는 유지한다.
